...
class DSFTrainer():
    ...
    def _label_function_t0(self, X, obs):
        ...
        with torch.no_grad():
            val = self.problem.p0(X)
            grad = self.problem.p0_grad(X)
            Ff = self.problem.dsf_F(X, val, grad)
    ...
    def _prepare_dataset_batched(self, chunk_size: int = None):
        ...
        while processed < total:
            ...
            with torch.no_grad():
                X_all, obs_chunk, _ = self.simulator.simulate_state_and_obs(n, self.model.n_obs + 1)

                # Auxiliary process: shuffle X within the chunk
            ...
            if self.model_number > 1:
                ...
                # compute how many repeats we can form in this chunk
                ...
                with torch.no_grad():
                    ...
                    likelihood = self.problem.obs_likelihood(X, obs_last).view(-1, 1)

                    # Likelihood gradient term
                    expanded_obs = (obs_last - self.problem.obs_batch(X)).unsqueeze(2)
                    H = self.problem.get_obs_jacobian(X)
                    Ht = torch.transpose(H, 1, 2)
                    likelihood_grad = (
                        likelihood.t() * torch.einsum('ijk,ikm->ijm', Ht, expanded_obs).transpose(2, 0)[0]
                    ).t() / (self.problem.obs_noise_std ** 2)

                    # Deep Splitting ingredients
                    val = likelihood * pred
                    grad = likelihood * pred_grad + likelihood_grad * pred

                    unique_obs = obs_expanded[::thin_out_factor]

                    def unnorm_density(X_last_in, obs_in):
                        o_last = obs_in[:, -1, :]
                        return self.prev_model(X_last_in, obs_in[:, :-1, :]) * \
                            self.problem.obs_likelihood(X_last_in, o_last).view(-1, 1)

                    norm_consts_unique = self.normalizer.get_nc(unnorm_density, unique_obs)
                    norm_const = norm_consts_unique.repeat_interleave(thin_out_factor, dim=0)

                    # Final Deep Splitting label (linear normalization)
                    Ff = self.problem.dsf_F(X, val, grad)
                    labels_chunk = (val + Ff * self.deltat) / norm_const 
            ...
            # Optional: proactively free GPU cache between chunks
            if torch.cuda.is_available():
                for name in ["Z", "X_all", "Z_label_chunk", "Z_input_chunk",
                            "obs_for_labels", "labels_chunk", "X", "pred", "pred_grad",
                            "obs_expanded", "obs_unique", "norm_const", "norm_consts_unique"]:
                    if name in locals():
                        del locals()[name]

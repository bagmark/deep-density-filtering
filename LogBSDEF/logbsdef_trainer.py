...
class LogBSDEFTrainer():
    ...
    def _train_one_batch(self, batch_size):
        ...
        # Now expand obs to match X: repeat each obs `thin_out_factor` times
        ...
        with torch.no_grad():
            if self.model.n_obs == 0:
                labels = -self.problem.log_p0(X[:, -1, :])
            else:
                # Get unique obs for computing normalization constants
                unique_obs = obs[::thin_out_factor]

                logfilter_unnorm = -self.prev_model.logfilter_unnorm(X[:, -1, :], obs)

                if self.prev_model.model_type == "FCN":
                    log_norm_consts = self.normalizer.get_log_nc(self.prev_model.logfilter_unnorm, unique_obs)
                else:
                    log_norm_consts = self.normalizer.get_log_nc(self.prev_model.logfilter_unnorm_dec, unique_obs, encode=self.prev_model.encode)

                log_norm_consts = (log_norm_consts).repeat_interleave(thin_out_factor, dim=0)

                labels = (logfilter_unnorm + log_norm_consts)

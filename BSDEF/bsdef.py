...
from BSDEF.bsdef_model import BSDEFModel
from BSDEF.bsdef_trainer import BSDEFTrainer
...
class BSDEF():
    def __init__(self, params, problem, times, obs_indices, normalizer):
        ...
        for n_obs in range(self.n_obs):
            ...
            self.models = self.models + [BSDEFModel(self.params, problem, n_obs, model_times)]
    ...
    def train(self, trainer_params, optimizer_params):
        ...
        for i, model in enumerate(self.models):
            ...
            prev_model.eval()
            model.train()
            trainer = BSDEFTrainer(trainer_params, self.simulator, model, prev_model, optimizer, scheduler, self.normalizer, self.problem)
            trainer.train_model()
    ...
    def evaluate_filter_pdf(self, obs_idx, x, obs, normalized=True):
        ...
        with torch.no_grad():
            ...
            if normalized:
                norm_consts = self.normalizer.get_nc(model.filter_unnorm, obs_spec_exp)
                filter_norm = filter_unnorm / norm_consts
                return filter_norm.view(-1)
            else:
                return filter_unnorm.view(-1)
    ...
    def get_filter_means_and_consts(self, obs):
        ...
        for i, model in enumerate(self.models):
            ...
            with torch.no_grad():
                obs_spec = obs[:(model.n_obs+1), :]
                norm_const, mean = self.normalizer.get_nc(model.filter_unnorm, obs = obs_spec.unsqueeze(0), get_fm=True)
                means[i, :] = mean
                norm_consts[i, :] = norm_const
